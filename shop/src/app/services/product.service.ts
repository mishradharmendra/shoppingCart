...
export class ProductService {
    ...
    private categoryUrl = `${apiUrl}/products/category`;

    constructor(private http: HttpClient) {
    }

    getAllInPage(page: number, size: number): Observable<any> {
        const url = `${this.productUrl}?keyword=&pageNumber=${page}&size=${size}`;
    ...
    update(productInfo: ProductInfo): Observable<ProductInfo> {
        const url = `${apiUrl}/seller/product/${productInfo._id}/edit`;
    ...
    delelte(productInfo: ProductInfo): Observable<any> {
        const url = `${apiUrl}/seller/product/${productInfo._id}/delete`;
