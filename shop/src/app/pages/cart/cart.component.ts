...
export class CartComponent implements OnInit, OnDestroy, AfterContentChecked {
    ...
    remove(productInOrder: ProductInOrder) {
        this.cartService.remove(productInOrder).subscribe(
            success => {
               this.productInOrders = this.productInOrders.filter(e => e.productId !== productInOrder._id);
        ...
        if (!this.currentUser) {
            ...
            this.cartService.checkout().subscribe(
                order => {
                    ...
                    let orderDetailURL = '/order/'+this.order;
                    this.router.navigate([orderDetailURL]);
                },
                error1 => {
